...
const CreateNewDeck = () => {
    ...
    const handleAddCard = (card) => {
        ...
        if (selectedCards.length < 20 && cardCount < 3) {
            setSelectedCards([...selectedCards, card]);
        } else if (cardCount >= 3) {
            alert(`No puedes agregar más de 3 copias de "${card.name}" al mazo.`);
        }
    };
    ...
    const handleRemoveCard = (card) => {
        ...
        if (index !== -1) {
            ...
            updatedCards.splice(index, 1); // Eliminar solo una instancia
    ...
    return (
        <div className={styles.createNewDeck}>
            ...
            <button
                disabled={deckTitle.trim() === '' || selectedCards.length !== 20}
